...
if __name__ == '__main__':
    ...
    for i in range(num_players):
        my_cluster1 = random.random()*100
        my_cluster2 = random.random()*100
        my_cluster3 = random.random()*100
        my_cluster4 = random.random()*100
        my_free_space_mult = random.random()*10
        my_free_space_exp = random.random()*10
        your_cluster1 = random.random()*100
        your_cluster2 = random.random()*100
        your_cluster3 = random.random()*100
        your_cluster4 = random.random()*100
        your_free_space_mult = random.random()*10
        your_free_space_exp = random.random()*10
        players.append(
    ...
    for i in range(num_generation):
        ...
        for k in range(num_rounds):
            ...
            for j in range(0, len(winners) - 1, 2):
                [child1, child2] = sbx(winners[j].eval_vector, winners[j+1].eval_vector, 20, 0, 100)
                mutated_child1 = pm(child1, 20, 0, 100, 1/len(child1))
                mutated_child2 = pm(child2, 20, 0, 100, 1 / len(child2))
